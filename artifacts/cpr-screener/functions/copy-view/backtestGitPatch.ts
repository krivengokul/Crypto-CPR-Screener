 ...
 * never touches backtest.ts on disk. This module does the same clone at
 * the SOURCE TEXT level, so the result can be committed straight to
 ...
 *
 * Unchanged from artifacts/cpr-screener/src/lib/backtestGitPatch.ts — kept
 * as a copy here (rather than imported) because this function deploys as
 * its own standalone unit, separate from the Next.js app's build.
 */
 ...
 * ORIGINAL condition, and adds `newKey` into whichever
 * BACKTEST_CATEGORIES `subPatternKeys` array currently contains
 * `sourceKey`, right next to it. Throws with a descriptive message on any
 * lookup failure — the caller should surface that message as-is rather
 * than a generic 500, since it's almost always "no such key" or "not
 * nested anywhere," both meaningful to whoever's clicking "Create copy".
 */
