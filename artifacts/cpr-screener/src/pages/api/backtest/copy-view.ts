 ...
 *
 * NOTE ON LOCATION: this file must live at
 *   artifacts/cpr-screener/src/pages/api/backtest/copy-view.ts
 * Next.js (pages router) only treats files under pages/api/ as serverless
 * API routes — anywhere else under pages/ and it's rendered as a page
 * component instead, and req.method / res.status(...).json(...) never run.
 * It was previously committed at pages/copy-view.ts, which would not have
 * worked as an endpoint.
 */
...
// Repo layout is artifacts/cpr-screener/src/... (this is not a repo-root Next.js app),
// so the default must include that prefix or the GitHub Contents API 404s.
